...
- Expo Router app with routes in `src/app`
- Styling is currently a mix of `StyleSheet` usage plus theme helpers and `src/global.css` for web fonts
- `pnpm-lock.yaml` and `pnpm-workspace.yaml` already exist, but the repo still behaves like a single-package app
- No dedicated unit test, E2E test, or CI workflow setup yet
...
- [ ] Complete the pnpm workspace monorepo migration
...
- [ ] Setup ESLint and Prettier for linting and formatting
...
- [ ] Switch styling to Uniwind using the current setup from `https://docs.uniwind.dev/`
...
- [ ] Setup CI/CD using GitHub Actions on pushes to `main` and selected branches
...
- [ ] Run the web and iOS builds to verify everything is working and fix issues
